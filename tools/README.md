...
```bash
node tools/log-shipper.js --key <apiKey> [--url http://localhost:4000] [--source myapp] -- <command> [args...]

# example
node tools/log-shipper.js --key ls_xxx --source myapp -- node myapp.js
```
...
```bash
node tools/demo-log-generator.js --key <apiKey> [--url http://localhost:4000] [--source demo-generator] [--interval-ms 1500] [--burst-chance 0.03]
```
