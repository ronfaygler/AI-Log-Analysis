...
Frontend on **Vercel**, `api` + `worker` on **Fly.io** (both need to run continuously — the worker holds a blocking Redis loop, which free-tier "web service" hosts like Render don't support without a paid plan), **MongoDB Atlas** and **Upstash Redis** for data.
...
```bash
cd api
fly apps create logsentinel-api   # app name from api/fly.toml; pick another if taken
...
Your API's public URL will be `https://logsentinel-api.fly.dev`.

### Deploy `worker`

```bash
cd ../worker
fly apps create logsentinel-worker
fly secrets set --app logsentinel-worker `
  MONGO_URI="mongodb+srv://..." `
  REDIS_URL="rediss://..." `
  REDIS_QUEUE_NAME="logsentinel:jobs" `
...
Use the **same** `MONGO_URI` and `REDIS_URL` for both apps — they share the database and queue.
...
- Re-deploy either Fly app after a code change with `fly deploy --app <name>` from that service's directory.
